...
/* ===== Input filter ===== */
$nama_wisata = $_POST['nama_wisata'] ?? $_GET['nama_wisata'] ?? '';
...
$filterParams = '';
if ($nama_wisata !== '') $filterParams .= '&nama_wisata=' . urlencode($nama_wisata);
if ($from_date !== '') $filterParams .= '&from_date=' . urlencode($from_date);
if ($to_date !== '') $filterParams .= '&to_date=' . urlencode($to_date);
?>
<!DOCTYPE html>
<html lang="en">
...
<body>
            ...
            <div class="col-lg-12 grid-margin stretch-card">
              <div class="card">
                <div class="card-body">
                  ...
                  <!-- Filter Tanggal -->
                  <form method="post" class="form-inline mb-3" action="?page=1<?= $filterParams ?>">
                  ...
                  <?php if ($totalPages > 1): ?>
                    <nav aria-label="Page nav" class="mt-4">
                      <ul class="pagination justify-content-center">
                        <?php if ($page > 1): ?>
                          <li class="page-item"><a class="page-link" href="?page=<?= $page - 1 . $filterParams ?>">«</a></li>
                        <?php endif;
                        $s = max(1, $page - 2);
                        $e = min($totalPages, $page + 2);
                        for ($i = $s; $i <= $e; $i++): ?>
                          <li class="page-item <?= $i == $page ? 'active' : '' ?>"><a class="page-link" href="?page=<?= $i . $filterParams ?>"><?= $i ?></a></li>
                        <?php endfor; ?>
                  ...
                  <!-- Filter Nama Wisata -->
                  <form method="post" class="form-inline mt-3" action="?page=1<?= 
                  ...
                  <h4 class="card-title mt-4">Grafik Jumlah Pengunjung</h4>
  ...
  <script>
    ...
    document.getElementById('exportPDF').onclick = () => {
      ...
      const head = [...document.querySelectorAll('#fullExportTable thead th')].map(th => th.textContent.trim());
      const body = [...document.querySelectorAll('#fullExportTable tbody tr')].map(tr => [...tr.querySelectorAll('td')].map(td => td.textContent.trim()));
      doc.text('Data Wisata (Export Semua)', 14, 15);
      doc.autoTable({ head: [head], body, startY: 22, theme: 'grid', styles: { fontSize: 8 } });
